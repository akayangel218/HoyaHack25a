...
const ClassifierCheng = () => {
    ...
    async function handleImageUpload(event) {
      ...
      setMessageList((prev) => prev.map(message => {
        ...
        if(message.id === count){
            console.log("check in if statement!!!", message)
          return {id: message.id, image: imageUrl, classification: response}
        }
        return message;
      }));
      setCount((prev) => prev + 1);
  ...
  async function chengModelQuery(imageFile) {
      ...
      setLoading(true);
        // fetch from backend
      const response = await fetch('http://10.150.237.146:5000/chengml/upload', {
        method: 'POST',
        body: formData,
      })
